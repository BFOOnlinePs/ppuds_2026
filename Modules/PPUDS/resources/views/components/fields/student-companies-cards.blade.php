            ...
            @foreach ($studentCompanies as $studentCompany)
                ...
                <a href="{{ route('student-companies.details', $studentCompany) }}"
                    ...
                    <div class="flex w-32 shrink-0 items-center justify-center bg-gray-50 p-4 dark:bg-gray-800 sm:w-40">
                        <img src="{{ $studentCompany->company?->getImageAttribute() }}"
                            alt="{{ $studentCompany->company?->name }}" class="h-24 w-24 rounded-lg object-contain">
                    </div>

                    <div class="flex min-w-0 flex-1 flex-col p-4">
                        <div class="mb-3 flex flex-wrap items-center gap-2">
                            <span
                                class="inline-flex rounded-md px-2 py-1 text-xs font-medium ring-1 {{ $statusClasses }}">
                                {{ $studentCompany->status?->getLabel() ?? __('Status') }}
                            </span>
                        </div>

                        <h3
                            class="truncate text-base font-semibold text-gray-950 transition group-hover:text-primary-700 dark:text-white dark:group-hover:text-primary-300">
                        ...
                        <span
                            class="mt-auto inline-flex items-center gap-1 pt-4 text-sm font-medium text-primary-600 dark:text-primary-400">
