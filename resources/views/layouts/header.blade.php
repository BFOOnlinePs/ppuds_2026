            ...
            <div class="horizontal-logo flex items-center justify-between ltr:mr-2 rtl:ml-2 lg:hidden">
                <a href="index.html" class="main-logo flex shrink-0 items-center">
                    <img class="inline w-8 ltr:-ml-1 rtl:-mr-1"
                        src="{{ $settingsModel->getLogoUrl() }}" alt="image" />
                    <span
...
{{--            </div>--}}
            <div x-data="header"
                ...
                <div class="dropdown shrink-0" x-data="{
                    open: false,
                    languages: {{ Js::from(
                        collect(LaravelLocalization::getSupportedLocales())->map(function ($props, $code) {
                                return [
                                    'value' => $code,
                                    'key' => $props['native'],
                    ...
                    <a href="javascript:;"
                        class="block rounded-full bg-white-light/40 p-2 hover:bg-white-light/90 hover:text-primary dark:bg-dark/40 dark:hover:bg-dark/60"
                        @click="open = !open">
                        <img src="{{ asset('assets/dashboard/admin-theme/images/flags/' . strtoupper(app()->getLocale()) . '.png') }}"
                            alt="Current Language" class="h-5 w-5 rounded-full object-cover" />
                    </a>

                    <!-- قائمة اللغات -->
                    <ul x-cloak x-show="open" x-transition
                        class="top-11 grid w-[280px] grid-cols-2 gap-y-2 !px-2 font-semibold text-dark ltr:-right-14 rtl:-left-14 dark:text-white-dark sm:ltr:-right-2 sm:rtl:-left-2 absolute z-50 bg-white dark:bg-[#1e293b] rounded shadow">
                        <template x-for="item in languages" :key="item.value">
                            <li>
                                <a :href="item.url"
                                    class="flex items-center hover:text-primary px-2 py-1 rounded transition-all duration-150"
                                    :class="{ 'bg-primary/10 text-primary': $store.app.locale === item.value }">
                                    <img class="h-5 w-5 rounded-full object-cover"
                                        :src="`{{ asset('assets/dashboard/admin-theme/images/flags') }}/${item.value.toUpperCase()}.png`"
                                        alt="flag" />
...
{{--                </div>--}}
                <div class="dropdown" x-data="dropdown" @click.outside="open = false">
                    ...
                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms
                        ...
                        @livewire(\Modules\Core\Livewire\HeaderNotification::class)
{{--                        @forelse($notifications as $notification)--}}
{{--                            <li class="dark:text-white-light/90">--}}
{{--                                <div class="group flex items-center px-4 py-2" @click.self="toggle">--}}
{{--                                    <div class="grid place-content-center rounded">--}}
{{--                                        <div class="relative h-12 w-12">--}}
{{--                                            <img class="h-12 w-12 rounded-full object-cover"                                                 :src="`assets/images/${notification.profile}`"--}}
{{--                                                 alt="image" />--}}
{{--                                            <span--}}
...
{{--                                            <h6 x-html="">{{ $notification->data['message'] }}</h6>--}}
                ...
                <div class="dropdown flex-shrink-0" x-data="dropdown" @click.outside="open = false">
                    <a href="javascript:;" class="group relative" @click="toggle()">
                        <span><img class="h-9 w-9 rounded-full object-cover saturate-50 group-hover:saturate-100"
                                src="{{ asset(auth()->user()->getAvatarUrlAttribute()) }}" alt="image" /></span>
                    </a>
                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms
                            ...
                            <div class="flex items-center px-4 py-4">
                                <div class="flex-none">
                                    <img class="h-10 w-10 rounded-md object-cover"
                                        src="{{ asset(auth()->user()->getAvatarUrlAttribute()) }}" alt="image" />
                                </div>
                        ...
                        @hasrole('Student')
                            <li>
                                <a href="{{ route('students.details', auth()->id()) }}" class="dark:hover:text-white" @click="toggle">
                        ...
                        <li class="border-t border-white-light dark:border-white-light/10">
                            <a href="#" class="!py-3 text-danger" @click.prevent="logoutAndToggle">
                                <svg
